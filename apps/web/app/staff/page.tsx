        ...
        <section className="panel" aria-labelledby="import-title">
          ...
          {importPreview && (
            <div className="import-preview" aria-live="polite">
              ...
              <div className="table-wrap"><table className="data-table"><thead><tr><th>Row</th><th>Employee</th><th>Name</th><th>Phone</th><th>Result</th><th>Details</th></tr></thead><tbody>{importPreview.rows.map((row) => <tr key={row.rowNumber}><td>{row.rowNumber}</td><td>{String(row.value.employeeNumber ?? "")}</td><td>{String(row.value.fullName ?? "")}</td><td>{String(row.value.phone ?? "")}</td><td><span className={`badge ${row.status === "CREATE" ? "ok" : row.status === "UPDATE" ? "submitted" : "rejected"}`}>{row.status.replace(/_/g, " ")}</span></td><td>{row.errors?.join("; ") ?? ""}</td></tr>)}</tbody></table></div>
